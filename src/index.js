...
function allowCustomBlockInTermTemplate( settings, name ) {
	if ( name !== 'core/term-template' ) {
		return settings;
	}

	const currentAllowed = settings.allowedBlocks || [];

	return {
		...settings,
		allowedBlocks: [ ...currentAllowed, 'term-thumbnails/term-thumbnail' ],
	};
}

addFilter(
	'blocks.registerBlockType',
	'term-thumbnails/allow-term-thumbnail-in-term-template',
	allowCustomBlockInTermTemplate
);
...
domReady( () => {
	 ...
	 * @return {HTMLElement} Thumbnail preview element.
	 */
	 ...
	 * @return {string} REST base for the taxonomy.
	 */
	function getRestBase( taxonomySlug ) {
		return (
			window.termThumbnails?.restBases?.[ taxonomySlug ] ?? taxonomySlug
		);
	}
	 ...
	 *   POST /wp/v2/{restBase}/{termId}  { meta: { term_thumbnail_id: attachmentId } }
	 *
	...
	function apiSetThumbnail( termId, attachmentId ) {
		...
		return apiFetch( {
			...
			method: 'POST',
			data: { meta: { term_thumbnail_id: attachmentId } },
		} );
	}
