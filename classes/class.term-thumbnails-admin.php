...
class Term_Thumbnails_Admin
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_delete-term-thumbnail', [$this, 'ajax_delete_term_thumbnail']);
        add_action('wp_ajax_get-term-thumbnail', [$this, 'ajax_get_term_thumbnail']);
        add_action('wp_ajax_set-term-thumbnail', [$this, 'ajax_set_term_thumbnail']);
    }
    ...
    {
        wp_register_script('term-thumbnails', plugins_url('../javascript/term-thumbnails.js', __FILE__), [], $this->version, true);
    ...
    public function edit_form_fields($tag)
    {
		...
		<tr class="form-field">
			...
			<td>
				...
				<a class="button remove-term-thumbnail" id="remove-term-thumbnail-<?php echo $term_id ?>" href="#" data-id-field="#term-thumbnail-id-<?php echo $term_id ?>" <?php if (!has_term_thumbnail($term_id)) {
            echo 'style="display: none"';
        } ?>><?php printf(__('Remove %s image', 'term-thumbnails'), $taxonomy); ?></a>
				<a class="button add-term-thumbnail" href="#" data-id-field="#term-thumbnail-id-<?php echo $term_id ?>" <?php if (has_term_thumbnail($term_id)) {
            echo 'style="display: none"';
        } ?>><?php printf(__('Set %s image', 'term-thumbnails'), $taxonomy); ?></a>

				<input name="term-thumbnail-id" value="<?php if (has_term_thumbnail($term_id)) {
            echo get_term_thumbnail_id($term_id);
        } ?>" id="term-thumbnail-id-<?php echo $term_id ?>" type="hidden">
			</td>
		</tr>
    ...
    public function manage_taxonomy_custom_column($content, $column_name, $term_id)
    {
        ...
        switch ($column_name) :
            ...
            case 'thumbnail':
                ...
                echo get_term_thumbnail($term_id, 'thumbnail', ['class' => 'term-thumbnail']); ?>

				<a class="button remove-term-thumbnail" href="#" data-ajax="1" data-id="<?php echo $term_id ?>" <?php if (!has_term_thumbnail($term_id)) {
                    echo 'style="display: none"';
                } ?>><?php printf(__('Remove %s image', 'term-thumbnails'), $taxonomy); ?></a>
				<a class="button add-term-thumbnail" href="#" data-ajax="1" data-id="<?php echo $term_id ?>" <?php if (has_term_thumbnail($term_id)) {
                    echo 'style="display: none"';
                } ?>><?php printf(__('Set %s image', 'term-thumbnails'), $taxonomy); ?></a>
    ...
    public function register_tax_hooks()
    {
        ...
        add_action('edited_'.$taxonomy, [$this, 'edit_term']);
        add_action('created_'.$taxonomy, [$this, 'created_term_thumbnail']);
        add_filter('manage_edit-'.$taxonomy.'_columns', [$this, 'manage_edit_taxonomy_custom_column']);
        add_filter('manage_'.$taxonomy.'_custom_column', [$this, 'manage_taxonomy_custom_column'], 10, 3);
